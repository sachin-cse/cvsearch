/*import superioAdaterPlugin from "../../../../resources/admin/js/ckeditor/uploadAdapter";*/
...
jQuery(function ($) {
    ...
    $(document).on("change", ".cv-manager-widget .file-edit-box input[name=csv_default]", function (e) {
        ...
        console.log(t.val());
        $.ajax({
            url: superio.url + '/user/set-default-cv',
            data: {
                'cv_id': t.val()
            },
            dataType: 'json',
    ...
    $(document).on("click", ".delete-cv", function (e) {
        ...
        if (confirm(i18n.confirm_delete)){
            t.addClass("loading");
            $.ajax({
                url: superio.url + '/user/delete-cv',
                data: {
    ...
    if($(".cv-drag-area").length > 0) {
        ...
        function uploadCvFile() {
            ...
            if (validExtensions.includes(fileType)) {
                ...
                $.ajax({
                    url: superio.url + '/user/upload-cv',
                    data: formData,
                    dataType: 'json',
                    type: 'post',
                    contentType: false,
...
var vendorPayout = {
    saveAccounts:function (btn) {
        ...
        $.ajax({
            url:superio.url+'/vendor/storePayoutAccounts',
            method:"post",
    ...
    sendRequest:function (btn) {
        ...
        $.ajax({
            url:superio.url+'/vendor/createPayoutRequest',
            method:"post",
