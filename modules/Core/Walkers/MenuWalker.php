	...
	class MenuWalker
	{
				...
				if($this->menu->location == 'primary' && !(is_candidate() && !is_admin())){
                    ...
                    if(is_employer()){
                        ...
                        <li class="mm-add-listing">
                            <a href="<?php echo route('user.create.job'); ?>" class="theme-btn btn-style-one <?php echo !auth()->check() ? 'bc-call-modal login' : ''; ?>"><?php echo __("Job Post"); ?></a>
                        </li>
                        <?php
                    }
                    echo ob_get_clean();
		...
		public function generateTree($items = [],$depth = 0,$parentKey = '')
		{
			...
			foreach ($items as $k=>$item) {
				...
				if (!empty($item['children'])) {
					$item['name'] .= ' <i class="caret flaticon-down-arrow"></i>';
				}
