...
class Gig extends Bookable
{
    ...
    public static function search($filters = [])
    {
        ...
        if(!empty($filters['category'])){
            $model_gig->where(function ($q) use($filters){
                $q->where('cat_id', $filters['category'])
                    ->orWhere('cat2_id', $filters['category'])
                    ->orWhere('cat3_id', $filters['category']);
            });
        }
        ...
        if(!empty($filters['cat3_id'])){
            $model_gig->where('cat3_id',$filters['cat3_id']);
        }
    ...
    public function sendNotifyToAuthor(GigOrder $gig_order){

        $user = $gig_order->author;

        $data = [
            'id' => $gig_order->id,
            'to'      => 'candidate',
            'name' => $gig_order->customer->getDisplayName() ?? '',
            'avatar' => $gig_order->customer->avatar_url ?? '',
            'link' => route('seller.order', ['id'=> $gig_order->id]),
            'type' => 'order_gig',
            'message' => __(':name ordered the gig ":gig"', ['name' => $gig_order->customer->getDisplayName() ?? '', 'gig' => $gig_order->gig->title ?? ''])
        ];

        $user->notify(new PrivateChannelServices($data));
    }

    public function sendNotifyToAdmin(GigOrder $gig_order){
        ...
        $data = [
            ...
            'name' => $gig_order->customer->getDisplayName() ?? '',
            'avatar' => $gig_order->customer->avatar_url ?? '',
            'link' => route('gig_order.admin.index'),
            'type' => 'order_gig',
            'message' => __(':name ordered the gig ":gig"', ['name' => $gig_order->customer->getDisplayName() ?? '', 'gig' => $gig_order->gig->title ?? ''])
        ];
