...
class GigManageController extends FrontendController
{
    ...
    public function create(Request $request){
        $this->checkPermission('gig_manage');
        $status = (!is_admin() && setting_item('gig_need_approve')) ? 'draft' : 'publish';
        $row = new Gig();
        $row->fill([
            'status' => $status
        ]);
    ...
    public function store(Request $request, $id){
        ...
        if(!is_admin() && setting_item('gig_need_approve')){
            $row->status = 'draft';
        }

        // Check Plan
        if($row->status == 'publish' and !auth()->user()->checkGigPostingMaximum()){
